...
A `case` statement at the bottom dispatches subcommands (`list`, `use`, `clean`, `new`, `edit`, `show`, `current`, `init`) to `cmd_*` functions. Helpers: `_profiles` (lists profile names), `_require_profile` (validates and sets `PROFILE_PATH`), `_is_claudio_link` (checks if a symlink points into the profiles dir), `_component_path` (maps component names to profile item paths).
